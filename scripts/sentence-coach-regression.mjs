...
// === B 方案：取消 meta intent，所有回顾性/追问性中文输入统一走 discussion ===
ok(
  detectStage3SentenceIntent("打磨哪里") === "discussion",
  "「打磨哪里」→ discussion（合并 meta 后由 LLM 自行判定要不要回放）",
);
ok(
  detectStage3SentenceIntent("错在哪") === "discussion",
  "「错在哪」→ discussion",
);
ok(
  detectStage3SentenceIntent("改哪") === "discussion",
  "「改哪」→ discussion",
);
ok(
  detectStage3SentenceIntent("哪里有问题") === "discussion",
  "「哪里有问题」→ discussion",
);
ok(
  detectStage3SentenceIntent("什么问题") === "discussion",
  "「什么问题」→ discussion",
);

// 备注：meta-style 回放交由 handle-turn 内的 buildDiscussionLlmResponse
// 处理——LLM 拿到 lastViabilityIssues 后自行判定要不要逐条复述。
// 这层 LLM 行为不在本回归脚本覆盖，集成测试见 e2e。
//
...
//   提示后续 reviewer：如果未来要加，必须先确认是「通用结构」而不是具体单词。
const caseBySentences = [
  // 缺冠词
  "Students want to pursue academic path after graduation.",
  // 语序反向 interested
  "Students should focus on subjects genuinely interested in.",
  // if 从句缺主语
  "if want to pursue an academic path, students should plan carefully",
];
for (const s of caseBySentences) {
  const r = assessLocalViability(s);
  ok(
    r.issues.length === 0,
    `本地不应在具体表达上堆 case：「${s.slice(0, 50)}...」无本地 issue`,
  );
}
