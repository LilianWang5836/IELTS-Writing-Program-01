...
async function reviewViabilityWithLlm(
  ...
  const prompt = [
    ...
    "",
    "【重要：你是唯一的语言层防线】",
    "本系统**不再**对具体词组/搭配写本地规则。任何具体表达层面的错误（缺冠词、",
    "搭配不当、语序反向、词性混淆、from-clause 缺主语、interested 用法等），",
    "只能由你来识别。本地只做「是否成句」的结构性判定，不再帮你兜底具体表达。",
    "→ 因此：凡是会让母语者愣一下的句子，必须至少报一条 issue 并定位到 anchor；",
    "  漏报具体表达错误，整个流程就把错误写入学生作文了。",
    "",
    "检查范围（必须覆盖，不要遗漏）：",
    "  hard 类（用户必须自己修，不能写入）：",
    ...
    "  • 指出哪里出了问题（关联 anchor）",
    "  • 用大白话解释「为什么不对 / 意思错位在哪」（讲意思，不讲语法术语）",
    "  • 用提问/反问让学生自己想（「想一想：…」「自己判断：…」）",
    "  • 禁止：直接给改后单词/短语/整句；禁止给候选词列表",
    "  • 禁止使用语法术语：系动词 / 定语从句 / 主动语态 / 被动语态 /",
    "    宾语补足语 / 修饰成分 / 表语 / 不定式作主语 / 过去分词 / 现在分词 等。",
    "    可以用的基础词：主语、动词、名词、形容词、副词、单数、复数、冠词、介词、时态、拼写。",
    "  • 禁止在 guideZh 里出现 「例如 X」「比如 they are」「应改为 X」「正确写法是 X」等暗示答案的措辞。",
    "  • 若必须解释含义错位，用大白话：「这听起来像 X 在做 Y」/「意思反了 / 主角错了」/「前后顺序需要换」。",
    "",
    "示例对照（务必模仿【好例】的写法）：",
    "",
    ...
    "",
    "  【错例】「subjects genuinely interested in」缺少修饰成分的主语和系动词（例如 they are），",
    "        或使用主动语态的定语从句（例如 that interest them）。",
    "  【好例】subjects 被放在 interested in 前面，听起来像是这些 subjects 自己在感兴趣——",
    "        但你想说的是「人」对这些 subjects 感兴趣，主角反了。想一想：怎么把「谁」在感兴趣说清楚？",
    "",
